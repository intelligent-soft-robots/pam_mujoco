...
def _from_template(template,locals_):
    ...
    items = ["position","color",
             "name","size","mass",
             "name_geom","name_joint","geom_type", # get_free_joint_body_xml
             "radius1","radius2", "color1", "color2",# get_goal_xml
             "index", "name_plate","name_net",
             "name_net_geom","name_plate_geom", # get_table_xml
             "name_inner","name_outer",
             "filename","xy_axes"] # get_robot_xml
...
def get_table_xml(name,position,size,color):
    ...
    template_geom_leg = str('<geom name="leg_$index$" pos= "$position$"'+
                            ' type="box" size="0.02 0.02 0.38" rgba="0.1 0.1 0.1 1.0"/>')
    template_geom_net = str('<geom name="$name_net_geom$" pos = "0.0 0.0 0.07625"'+
                       ' type="box" size="0.7825 0.02 0.07625" rgba="0.1 0.1 0.1 0.4"/>')

    name_plate_geom = name+"_plate"
    name_net_geom = name+"_net"

    xml = [_from_template(template_body,locals()),
...
def get_robot_xml(name,position,xy_axes):
    ...
    # xml file
    filename = paths.write_robot_body_xml(name)
    ...
    if xy_axes is not None:
        ...
        template = str('<body name="$name$" pos="$position$">\n'+
                       '<include file="$filename$"/>\n'+
                       '</body>\n')
...
def get_contacts_xml(robots,balls,tables,
                     solrefs,gaps):
    ...
    for ball in balls:
        ...
        for robot in robots:
            ...
            contacts.append(get_xml(ball.geom,robot.geom_racket_handle,"ball","racket_handle"))
        for table in tables:
