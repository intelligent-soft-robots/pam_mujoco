...
ContactCase get_contact_case4()
{
    ...
    set_values(
        ccase.previous.ball_velocity, -1.25239e-05, 3.60857e-05, -0.942676);
...
{
    RecomputeStateConfig config = get_table_recompute_config();
    ...
    {
        recompute_state_after_contact(
            config, c.previous, c.current, get_position, get_velocity);
