...
void PressureController<QUEUE_SIZE, NB_DOFS>::apply(const mjModel* m, mjData* d)
{
    ...
    // check if we must reset to initial state
    ...
    if (this->must_update(d))
    {
        ...
        // keeping in memory the first state ever observed, in order
        // for the reset function to work
        if (!initialized_)
        {
...
template <int QUEUE_SIZE, int NB_DOFS>
void PressureController<QUEUE_SIZE, NB_DOFS>::reset(const mjModel* m, mjData* d)
{
