...
void MirrorRobot<QUEUE_SIZE, NB_DOFS>::apply(const mjModel* m, mjData* d)
{
    ...
    // check if reset to initial test has been requested
    ...
    if (this->must_update(d))
    {
        ...
        // keeping in memory first state ever observed,
        // for the reset method to work
        if (!initialized_)
        {
            initialized_ = true;
...
template <int QUEUE_SIZE, int NB_DOFS>
void MirrorRobot<QUEUE_SIZE, NB_DOFS>::reset(const mjModel* m, mjData* d)
{
