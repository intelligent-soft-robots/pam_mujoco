...
{
    internal::save_state(
            d, index_q_robot_, 0, 0, index_geom_, racket_state_fk_update_);

    for (int dim = 0; dim < 3; dim++)
    {
        robot_fk_.set_position(dim, racket_state_fk_update_.contactee_position[dim]);
    }
    for (int dim = 0; dim < 3; dim++)
    {
        robot_fk_.set_velocity(dim, racket_state_fk_update_.contactee_velocity[dim]);
    }
    for (int dim = 0; dim < 9; dim++)
    {
        robot_fk_.set_orientation(dim, racket_state_fk_update_.contactee_orientation[dim]);
    }
}
