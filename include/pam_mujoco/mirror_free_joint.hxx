...
{
    // resetting the system if requested
    // (i.e. setting it back to the first observed state)
    if (this->reset_requested(segment_id_))
    {
        reset(m, d);
    }
    ...
    if (this->must_update(d))
    {
        ...
        if (!initialized_)
        {
            // keeping in memory the first state observed:
            // will allow the reset method to work
            initial_states_ = read_states_;
            initialized_ = true;
        }
    }
...
template <int QUEUE_SIZE>
void MirrorFreeJoint<QUEUE_SIZE>::reset(const mjModel* m, mjData* d)
{
    // purge all o80 commands and replace them by a command setting
