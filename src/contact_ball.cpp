...
void ContactMode::init(int index_qpos,
                       int index_qvel,
                       int index_geom,
...
bool ContactMode::contact_active(const mjModel* m, mjData* d)
{
    ...
    // contacts are neither "active" (from a contact detected in a previous
    // iteration) or "muted" (because a contact has been applied recently).
    // Evaluating things anew.
    bool mujoco_contact =
...
void ContactBall::init(const mjModel* m)
{
    ...
    {
        index_robot_qpos_ =
            m->jnt_qposadr[mj_name2id(m, mjOBJ_JOINT, robot_base_.c_str())];
    }
    contact_mode_.init(
        index_qpos_, index_qvel_, index_geom_, index_geom_contactee_);
}
    ...
    for(int index=0;index<3;index++)
        {
            ...
            if(std::abs(d->qvel[index_qvel_+index])<thres ||
    std::abs(d->qpos[index_qpos_+index])<thres)
                {
                    skip=false;
                    break;
                }
...
internal::ContactStates ContactBall::update_positions(
    const mjData* d, const internal::ContactStates& previous)
{
    return internal::update_positions(d,
                                      index_robot_qpos_,
                                      index_qpos_,
                                      index_qvel_,
                                      index_geom_contactee_,
...
void ContactBall::execute(const mjModel* m, mjData* d)
{
    ...
    if (!contact)
    {
        if (update)
        {
            ...
            double d_ball_contactee =
                mju_dist3(&(d->qpos[index_qpos_]),
                          &(d->geom_xpos[index_geom_contactee_ * 3]));
    ...
    contact_mode_.set_contact_overwrite(
        d, overwrite_ball_position_, overwrite_ball_velocity_);
