...
void add_share_robot_state(std::string segment_id, std::string robot_joint_base)
{
                            ...
                            ContactItems contact_item)
{
    std::shared_ptr<ContactBall> cb = std::make_shared<ContactBall>(
        segment_id, index_qpos, index_qvel, geom, contactee_geom, contact_item);
...
void execute(std::string mujoco_id, std::string model_path)
{
    ...
    // vector pam_mujoco::Controllers::controllers_ with instances of
    // controllers. The pam_mujoco::Controllers::apply has these controllers
    // being called sequentially at each mujoco iteration). (same principle for
    // biases, also encapsulated by the Controllers static class)
