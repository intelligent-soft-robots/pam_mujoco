...
bool ControllerBase::reset_requested(const std::string& segment_id) const
{
    // allows client to request a reset (i.e. back to initial state)
    // by setting the shared memory segment to true
    bool must_reset;
    shared_memory::get<bool>(segment_id, "reset", must_reset);
    if (must_reset)
    {
        shared_memory::set<bool>(segment_id, "reset", false);
        return true;
    }
    return false;
}
