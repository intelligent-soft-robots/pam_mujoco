...
MujocoStatePrinter::MujocoStatePrinter(std::string filepath)
    : m_file_(filepath),
      m_thread_(&MujocoStatePrinter::write_to_file, this),
      m_exit_(false)
{
    if (!m_file_) throw std::runtime_error("Could not open file");
    m_file_ << "time_stamp"
            << ","
            << "robot_position_0"
            << ","
...
void MujocoStatePrinter::write_to_file()
{
    while (true)
    {
        ...
        if (m_exit_ && m_queue_.empty()) return;
        ...
        m_file_ << std::fixed << std::setprecision(3) << state.time_stamp << ','
                ...
                << ',' << state.robot_positions[2] << ','
                << state.robot_positions[3] << ',' << state.robot_velocities[0]
                << ',' << state.robot_velocities[1] << ','
                << state.robot_velocities[2] << ',' << state.robot_velocities[3]
                << ',' << state.ball_positions[0] << ','
                << state.ball_positions[1] << ',' << state.ball_positions[2]
                << ',' << state.ball_velocities[0] << ','
                << state.ball_velocities[1] << ',' << state.ball_velocities[2]
                << '\n';
    }
}

std::string format_filename(const std::string& folder,
                            const std::string& filename)
{
    ...
    oss << folder << '/' << std::put_time(now_tm, "%Y_%m_%d_%H_%M_") << filename
        << ".csv";

    return oss.str();
}

MujocoStatePrinterController::MujocoStatePrinterController(
...
void MujocoStatePrinterController::init(const mjModel* m)
{
    if (qpos_ball_ < -1)
    {
        ...
        {
            qpos_ball_ =
                m->jnt_qposadr[mj_name2id(m, mjOBJ_JOINT, ball_joint_.c_str())];
            qvel_ball_ =
                m->jnt_dofadr[mj_name2id(m, mjOBJ_JOINT, ball_joint_.c_str())];
        }
        else
        {
            qpos_ball_ = -1;
        ...
        {
            qpos_robot_ = m->jnt_qposadr[mj_name2id(
                m, mjOBJ_JOINT, robot_joint_.c_str())];
            qvel_robot_ =
                m->jnt_dofadr[mj_name2id(m, mjOBJ_JOINT, robot_joint_.c_str())];
        }
        else
        {
            qpos_robot_ = -1;
